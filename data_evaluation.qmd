...
(classified_trigrams <- trigrams_df  %>%
   mutate(
     type = case_when(
       str_detect(key, r"(^[a-z][a-z][a-z]$)") ~ "letter→letter→letter",
       #str_detect(key, r"(^[a-z]\.<space>$)")  ~ "letterDotSpace",
       #str_detect(key, r"(^\.<space>[A-Z]$)")  ~ "dotSpaceLetter",
       str_detect(key, r"(^\.<space><shift)")  ~ "dot→space→shift",
       #str_detect(key, r"(^[a-z]\.<enter>$)")  ~ "letterDotEnter",
       #str_detect(key, r"(^\.<enter>[A-Z]$)")  ~ "dotEnterLetter",
       str_detect(key, r"(^\.<enter><shift)")  ~ "dot→enter→shift",
       str_detect(key, r"(^\,<space><shift)")  ~ "komma→space→shift",
       str_detect(key, r"(^\,<space>[a-z])")  ~ "komma→space→letter",
       TRUE ~ "all other"
     ),
     type = factor(type, levels = c("dot→enter→shift", "dot→space→shift", "komma→space→shift", "komma→space→letter", "letter→letter→letter", "all other"))#, ordered = TRUE)
